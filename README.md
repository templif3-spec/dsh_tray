...
- **托盘图标右键菜单**：
  ...
  - `dsh 设置…` — 配置监听地址（host）与端口、开机自启、自动更新，保存后生成启动覆盖层，可立即重启生效
  - `检查 dsh 更新…` — 比对本地版本与 npm registry 最新版本；有新版时询问是否立即更新（更新后询问是否重启 dsh 生效）
  - `退出` — 退出托盘程序（**不关闭 dsh**）
...
- **手动检查**：右键菜单 → `检查 dsh 更新…`，显示「已是最新 / 发现新版本 x.y.z」，可选择立即更新
- **自动更新**：设置中勾选「启动时自动检查并更新 DeepSeek Harness」后，程序启动 15 秒后静默检查，发现新版**自动执行更新**
- 更新方式：`npx --yes @deepseek-ai/dsh@latest --version`（把最新版取到 npx 缓存；托盘启动 dsh 时按"最新修改的缓存目录"自动指向新版本），随后询问是否重启 dsh 生效
- 更新需要网络；离线或受限环境下检查会提示失败，不影响其他功能
